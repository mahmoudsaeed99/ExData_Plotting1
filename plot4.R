...
with(subdata,{
  ...
  plot(subdata$Time , subdata$Sub_metering_1,type = "n",xlab="",ylab="Energy sub metering")
    with(subdata , lines(Time , as.numeric(as.character(subdata$Sub_metering_1))))
    with(subdata , lines(Time , as.numeric(as.character(subdata$Sub_metering_2)),col = "red"))
    with(subdata , lines(Time , as.numeric(as.character(subdata$Sub_metering_3)), col= "blue"))
    legend("topright" , lty =1 , col = c("black" , "red" , "blue") , legend = c("Sub_metering_1",
                                                                              "Sub_metering_2",
                                                                              "Sub_metering_3"),
         cex = 0.6)
  plot(subdata$Time , as.numeric(as.character(subdata$Global_reactive_power)) , type="l",
